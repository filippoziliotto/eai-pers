...
### `Baseline 2`: Random Point where map has non-zero elements
...
### `Baseline 3`: Query map using simple VLFM
In this baseline, we only evaluate the results by calculating the similarity between the original map and the embedded query
So we are not taking into account the multihead attention part. Results show that this is equivalent to a random center point baseline.
